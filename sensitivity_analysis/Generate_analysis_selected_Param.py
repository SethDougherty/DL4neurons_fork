# -*- coding: utf-8 -*-
"""
This python program also takes the parameter index that is to be modifed as input insted of all parameters 
Other input parameters are mType eType and number of samples.
Number of regions is hard coded

The range of increase is also increased (Default_param * e^x) for each region.

We are parlellising the nsamples but the parameters that are modified are in serial
"""
...
def main_for_divided_range():
    ...
    if(len(sys.argv)>4):
        ...
        print("ParamValue not found taking 0")
    print(procid)
    my_model = get_model('BBP',log,m_type=m_type,e_type=e_type,cell_i=0)
    ...
    for p_index in procid:
        ...
        pkl_fn =files_loc + str(nregions) + 'regions_' + m_type + '_' + e_type + adjusted_param + '_' + str(int(os.environ['SLURM_PROCID'])) + '.pkl'
        with open(pkl_fn, 'wb') as output:
            pkl.dump(all_volts,output)
    #p_ind = procid%(len(pnames))
    # adjusted_param = my_model.PARAM_NAMES[p_ind]
